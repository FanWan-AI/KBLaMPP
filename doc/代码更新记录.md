...
#### 代码改动内容
- **gen_world_llm.py**：
  - 在事实提示模板中硬性要求时间窗口落在2005-01-01至2024-12-31，并禁止重复的“头实体+关系+尾实体+起始日期”组合。
  - 新增过滤器，在解析每个批次后校验实体引用、关系名称、时间区间及重复项，并为合格事实分配连续的关系ID；被丢弃的事实会按“重复/超范围/无效”分类计数并写入日志。
  - 若整个批次因重复或超范围被淘汰，脚本会自动减半请求的facts_per_call（直至min_facts_per_call）并重试，避免多次交互后仍卡死。
  - 维护全局去重哈希表，保证最终写入JSONL的事实集合在逻辑上唯一、时间也满足阶段A默认配置期望。

#### 测试验证
- 运行 `python offline/gen_world_llm.py --help` 验证命令行接口更新。
- 建议以 `python offline/gen_world_llm.py --num_entities 18 --num_facts 80 --facts_per_call 16 --min_facts_per_call 4 \` 等参数重新拉取样本，观察日志中“rejected duplicate/out_of_range”统计是否符合预期，并确认最终JSONL长度仍与目标一致。

#### 后续推进
1. 结合新的过滤器输出，继续实现问答生成脚本/说明。
2. 根据stageA日期需求，如需其它时间段，可通过命令行参数或配置扩展DATE_MIN/DATE_MAX常量。

现已将去重机制与时间线校验直接集成至gen_world_llm.py，确保每次运行仅保留2005-2024年间不重复的事实数据，并在LLM输出异常时自动缩减批次进行重试。

**核心增强功能**：

1. **可复用工具模块**：新增日期解析与哈希追踪工具，配合全新实现的`filter_and_normalize_facts`流程，该流程具备：
   - 实体ID解析与标准化
   - 强制限定时间范围为2005-01-01至2024-12-31的ISO日期
   - 基于（头实体、关系、尾实体、起始时间）四元组的去重机制
   - 实时记录每批次被剔除数据的统计信息（重复/超范围/无效数据量）

2. **动态重试机制**：优化事实生成提示词，明确要求2005-2024时间范围并禁止重复组合。当整批数据被过滤时，系统自动降低单次生成数量（最低可至`--min_facts_per_call`设定值），确保脚本持续运行而非解析失败后崩溃

3. **文档与测试**：
   - 更新代码更新记录.md至v1.8版本，详述新增控制参数及重运行指令（`python gen_world_llm.py … --min_facts_per_call 4`）
   - 通过命令行验证（`python gen_world_llm.py --help`）确认新参数与默认值正常显示

**后续推进计划**：

1. 重新生成世界数据（使用原有命令），观察日志中`[gen_world_llm] facts_batchXX: rejected …`输出行以验证过滤器生效状态，最终JSONL文件应仍精确包含预期数量的事实数据

2. 启动问答生成配套工具（或适配gen_synth_qa.py），基于优化后的世界数据生成data/qa/synth_world_llm_{train,dev}.jsonl

3. 若阶段A需适配其他时间范围，可通过扩展脚本读取配置文件/命令行参数中的DATE_MIN/MAX实现——现有过滤框架已具备参数化扩展能力

### V1.8.1 LLM生成日期窗口热修复
- 2025-11-20 17:30

#### 代码改动内容
- 将 `offline/gen_world_llm.py` 中事实提示模板的时间窗口描述改为 `2005-01-01 ~ 2024-12-31`，与验证器保持一致，避免模型持续生成超范围日期导致事实被全部丢弃。
- 为最小批次引入失败上限：当 `--min_facts_per_call` 下连续 `--max_attempts` 次无合格事实时立即抛出异常并指示检查提示/过滤器，防止脚本在0进度时无限循环。

#### 测试建议
1. 重新执行 `python offline/gen_world_llm.py --num_entities 18 --num_facts 80 --facts_per_call 16 --min_facts_per_call 4`，观察日志中 `out_of_range` 是否大幅下降。
2. 如仍出现连续失败，可适当降低 `--facts_per_call` 或放宽日期范围常量，再次运行以定位问题。

### V1.9 LLM生成器时间窗对齐
- 2025-11-24 11:40

#### 代码改动内容
- `offline/gen_world_llm.py`
   - 新增 `--date-min/--date-max/--world-theme` 三个CLI参数，允许按配置指定可用年份区间与故事主题，并在日志中明确提示当前年份窗口。
   - 实体提示模板强化了“2005-2024时间窗”文字说明，要求每个summary提及有效年份；若解析出的年份超出区间，脚本会自动丢弃该批实体并重新请求，避免后续事实被整体判定为超范围。
   - 事实提示模板同步注入时间窗与主题，并强调若原始设定发生在更未来的年份必须“回溯”到允许范围内的早期版本。
   - 过滤器与日期校验逻辑改为读取动态日期参数，彻底移除写死的DATE_MIN/DATE_MAX常量；在最小批次下的空结果会触发明确信息提示，便于调试。
- `doc/代码更新记录.md`：记录本次修复细节与测试步骤。

#### 测试建议
1. 以默认参数运行：
    ```bash
    python offline/gen_world_llm.py --num_entities 18 --num_facts 80 --facts_per_call 16 --min_facts_per_call 4 \
       --date-min 2005-01-01 --date-max 2024-12-31 --world-theme "Post-2005 Earth-Mars joint missions"
    ```
    观察实体日志是否再出现>2024的年份，确保事实批次不再全部因`out_of_range`被拒绝。
2. 若需扩展其他时间段，只需调整CLI日期或在配置脚本中传参；过滤器会自动遵循新的年份窗口。
...
- `offline/gen_synth_qa.py`
   - 支持同时读取`.json`与`.jsonl`格式的世界文件，并在QA样本中写入`supported_relations`字段以保留关系ID。
   - 调整提问模板，统一引用“what did HEAD RELATION?”句式，并兼容multi-hop衔接逻辑。
- `configs/synth_world.yaml`：将数据路径切换为LLM生成的五元组/QA（`synth_world_llm*.jsonl`），并将store根目录迁移至`store/synth_world_llm`。
- `offline/encode_kv.py`：在无法联网下载句向量模型时自动降级为确定性随机嵌入，确保流水线离线可跑；日志会明确提醒“encoder offline”。
- `train/train_stageA.py`：新增`--max_steps`覆盖参数，便于执行短程冒烟测试。

#### 数据与流水线
- 通过 `python offline/gen_world_llm.py --num_entities 18 --num_facts 80 --facts_per_call 16 --min_facts_per_call 4 --date-min 2005-01-01 --date-max 2024-12-31 --world-theme "Post-2005 Earth-Mars joint missions"` 生成了新的世界数据，输出位于 `data/5tuple/synth_world_llm.jsonl` 与 `data/raw/synth_world_llm.json`。
- 使用改进版QA脚本分别生成 `data/qa/synth_world_llm_train.jsonl`（200条）与 `data/qa/synth_world_llm_dev.jsonl`（50条）。
- 运行 `python offline/run_pipeline.py --config configs/synth_world.yaml --steps encode index --force`，由于无法访问HuggingFace，编码阶段自动降级为随机嵌入，但FAISS索引已依据新五元组构建完毕。

#### 阶段A冒烟
- 通过 `python -m train.train_stageA --config configs/synth_world.yaml --max_steps 50` 触发训练循环，脚本成功读取新QA与store，但因离线环境无法下载 `meta-llama/Llama-3.2-1B-Instruct`，在加载主干模型时失败。若需本地测试，可临时改用已缓存或离线可用的轻量模型，并在backbone配置中指向相应权重。
